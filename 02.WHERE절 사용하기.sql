...
-- OERDER BY 열 + ASC or DESC 열의 오름차순 또는 내립차순 정렬
SELECT *
FROM employees
...
SELECT *
FROM employees
ORDER BY employee_id; --ASC 기본 정렬은 생략 가능

-- 정렬 열이 2개 이상 일 때 첫번째 열로 정렬 후 두번째 열 정렬
SELECT department_id, employee_id, first_name, last_name
FROM employees
ORDER BY department_id, employee_id;

--별칭으로 정렬nt_id, last_name, salary*12 연봉
FROM employees
SELECT departme
ORDER BY 연봉 DESC;
